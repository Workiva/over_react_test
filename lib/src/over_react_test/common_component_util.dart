...
void testRequiredProps(BuilderOnlyUiFactory factory, dynamic childrenFactory(),
  ...
  Function(String propKey) _propTest;
  ...
  if (!isComponent2) {
    ...
    _propTest = (String propKey) {
      ...
      if (!reactComponentFactory.defaultProps.containsKey(propKey)) {
        testPropTypesWithUiProps(componentProps: (factory()
          ..remove(propKey)
        ), childProps: childrenFactory, shouldCheckConsoleErrorCount: false);
      }
  ...
  test('logs the correct errors when the required prop is not set or is '
      'null', () {
    requiredProps.forEach(_propTest);
  });
