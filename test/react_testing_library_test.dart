...
import 'testing_library/dom/top_level_queries_test.dart' as top_level_queries_test;
import 'testing_library/dom/wait_for_test.dart' as wait_for_test;
import 'testing_library/dom/within_test.dart' as within_test;
import 'testing_library/react/render_test.dart' as render_test;
import 'testing_library/util/matchers.dart';
import 'testing_library/util/rendering.dart';
...
main() {
  ...
  group('Testing Library', () {
    ...
    render_test.main();
    within_test.main();
    top_level_queries_test.main();

    group('', () {
      rtl.RenderResult renderResult;
      setUp(() {
        renderResult = rtl.render(elementsForQuerying('foo'));
      });

      test('foo', () async {
        expect(() async => await renderResult.findByTestId((content, el) => content != 'foo'),
            toThrowErrorMatchingInlineSnapshot(contains('foo'), endsWith(rtl.prettyDOM(renderResult.container))));
      });
    });
